  ...
  const contents = [
    ...
    <div key="wishes" className="space-y-4 sm:space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        {[
          { icon: '💖', text: 'Luôn tràn đầy yêu thương và hạnh phúc', color: 'from-pink-500 to-rose-500' },
          { icon: '✨', text: 'Tỏa sáng và thành công trong cuộc sống', color: 'from-purple-500 to-indigo-500' },
          { icon: '🌸', text: 'Mãi mãi xinh đẹp và tươi trẻ', color: 'from-red-500 to-pink-500' },
          { icon: '🎯', text: 'Đạt được mọi ước mơ em khao khát', color: 'from-blue-500 to-cyan-500' },
          { icon: '💪', text: 'Luôn khỏe mạnh và tràn đầy năng lượng', color: 'from-green-500 to-emerald-500' },
          { icon: '🥰', text: 'Mỗi ngày đều là một ngày tuyệt vời', color: 'from-amber-500 to-orange-500' },
        ].map((wish, index) => (
          <motion.div
            key={index}
            ...
            <div className="flex border-b border-white/20 bg-black/20">
              {tabs.map((tab, index) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className={`flex-1 py-3 sm:py-4 md:py-6 px-2 sm:px-4 md:px-6 text-center transition-all duration-300 relative ${
                    currentTab === index
